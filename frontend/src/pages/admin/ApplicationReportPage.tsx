...
import { type ApplicationReport, type AttemptDetail, getApplicationReport } from '../../api/admin'
...
export default function ApplicationReportPage() {
  ...
  if (isLoading || !report) {
    return (
      <AdminPageWrapper>
        <p>Загрузка отчёта...</p>
        <button className="btn btn-secondary" onClick={() => navigate('/admin/applications')}>
          Назад
        </button>
      </AdminPageWrapper>
    )
  ...
  return (
    <AdminPageWrapper>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        <p>
          <strong>Компетенция:</strong> {report.competencyName ?? '—'}
        </p>
        <p>
          <strong>Статус:</strong> {renderStatus(report.sessionStatus)}
        </p>
        <p>
          <strong>Создана:</strong> {formatDate(report.createdAt)}
        </p>
        <p>
          <strong>Завершена:</strong> {formatDate(report.updatedAt)}
        </p>
      </div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <h2>
          Результат:{' '}
          <span style={{ color: report.passed ? '#28a745' : '#dc3545', fontWeight: 700 }}>
      ...
      {report.competencies.length === 0 ? (
        ...
        report.competencies.map((comp) => (
          <div key={comp.topicId} className="card" style={{ marginBottom: '1rem' }}>
            ...
            <p>
              <strong>Средний балл:</strong> {comp.averageScore.toFixed(2)}
            </p>
            <p>
              <strong>Результат:</strong>{' '}
              <span style={{ color: comp.passed ? '#28a745' : '#dc3545', fontWeight: 600 }}>
                {comp.passed ? 'Пройден' : 'Не пройден'}
              </span>
            </p>
            {comp.followUpScores.length > 0 && (
              <p>
                <strong>Баллы за уточнения:</strong>{' '}
                {comp.followUpScores.map((s) => s.toFixed(2)).join(', ')}
              </p>
            )}
            ...
            return (
              <div key={main.attemptId} className="card" style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ marginTop: '0.5rem' }}>
                  ...
                  <div
                    className="rich-content"
                    style={{ backgroundColor: '#f8f9fa', padding: '0.75rem', borderRadius: '4px' }}
                ...
                >
                  ...
                  <span>
                    <span
                      className="info-hint"
                      data-tooltip="Уверенность ИИ в собственной оценке: HIGH — уверен, ответ однозначен; MEDIUM — умеренно; LOW — не уверен, ответ неоднозначен"
                    >
                  ...
                  <span>
                    <span
                      className="info-hint"
                      data-tooltip="Оценка засчитана: да — если балл > 0 (ответ по теме), нет — если 0 (ответ некорректный, не по теме)"
                    >
                      <strong>Валидна</strong>
                    </span>
                    : {main.validJudge === false ? 'нет' : 'да'}
                  </span>
                </div>
                ...
                {main.feedback && (
                  <div style={{ marginTop: '0.5rem' }}>
                    ...
                    <div
                      className="rich-content"
                      style={{ backgroundColor: '#eef', padding: '0.75rem', borderRadius: '4px' }}
                    ...
                    {children.map((fu: AttemptDetail) => (
                      ...
                      >
                        ...
                        <div style={{ marginTop: '0.25rem' }}>
                          ...
                          <div
                            className="rich-content"
                            dangerouslySetInnerHTML={{ __html: plainTextToHtml(fu.questionText) }}
                          />
                        </div>
                        <div style={{ marginTop: '0.25rem' }}>
                          <strong>Ответ сотрудника:</strong>
                        ...
                        >
                          ...
                          <span>
                            <strong>Уверенность:</strong> {fu.confidence ?? '—'}
                          </span>
                        </div>
                        {fu.feedback && (
                          <div style={{ marginTop: '0.25rem' }}>
